...
// DialogBox Variant
class ProgressDialogBox<T extends ProgressStatusI> implements ProgressVisualizerI {
...
// ProgressBar Variant
class ProgressStatusBarItem<T extends ProgressStatusI> implements ProgressVisualizerI {
  ...
  public progress(status: T) {
    this.statusBarItem.text = `${this.getIconPlaceholder()} ${this.title} (${status.steps}/${
      status.stepsMax
    }) ${this.getAsciiProgress(status)}`.trimLeft();
