...
export function using<T extends DisposableI>(resource: T, func: (resource: T) => void) {
  try {
    func(resource);
  } finally {
    resource.dispose();
...
export class HasDisposables implements DisposableI {
  protected disposables = new DisposableArray();
  public dispose() {
