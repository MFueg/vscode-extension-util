...
import { visualizerType } from '../progress/progress';
import { DisposableI } from '../disposable';
import { switchOs } from '../helper';
import { LogLevel } from '../logger';
import { CanNotifyMessages, NotifyMessageHandler } from '../message';
...
export class Configuration extends CanNotifyMessages implements DisposableI {
  ...
  private createCoreWatch() {
    ...
    this.coreConfigWatch = vscode.workspace.onDidChangeConfiguration(
      (configChange: vscode.ConfigurationChangeEvent) => {
        let affects = (property: Property): boolean => {
          return configChange.affectsConfiguration(this.fullname(property), this.workspaceFolder.uri);
        };
